package com.completeperspective.display
{
	...
	public class Reflection extends MovieClip
	{
		...
		public function Reflection(args:Object){
			...
			//get a reference to the movie clip = cast the DisplayObject that
			//is returned as a MovieClip
			...
			//set the height of the matrix for the gradient mask
			...
			//if we are updating the reflection for a video or animation 
			if( updateTime > -1 ) {
		...
		{
			//allows the user to set the ara that the reflection is allowed
			bounds.width = w;
			bounds.height = h;
